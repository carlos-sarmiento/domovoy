...
class EventListenerCallbackFull(Protocol):
    def __call__(*, event_name: str, data: dict[str, HassValue]) -> None | Awaitable[None]:
        ...


# Single Parameter Callback
class EventListenerCallbackWithEventName(Protocol):
    def __call__(*, event_name: str) -> None | Awaitable[None]:
        ...


class EventListenerCallbackWithEventData(Protocol):
    def __call__(*, data: dict[str, HassValue]) -> None | Awaitable[None]:
        ...
